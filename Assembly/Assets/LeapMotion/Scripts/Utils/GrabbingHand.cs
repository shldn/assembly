...
public class GrabbingHand : MonoBehaviour {
	...
	public Transform grabEffect = null;
	public AudioClip grabClip = null;

	protected bool clenchedFist = false;
	public bool ClenchedFist { get { return clenchedFist; } }
	...
	HandModel hand_model = GetComponent<HandModel>();
		...
		object_pinch_offset_ = Quaternion.Inverse(active_object_.transform.rotation) * object_pinch_offset_;
		rotation_from_palm_ = Quaternion.Inverse(palm_rotation_) * active_object_.transform.rotation;

	if(grabEffect)
		Instantiate(grabEffect, filtered_pinch_position_, Quaternion.identity);

	if(grabClip)
		AudioSource.PlayClipAtPoint(grabClip, filtered_pinch_position_);
	...
	protected void UpdatePinchPosition() {
		HandModel hand_model = GetComponent<HandModel>();
		current_pinch_position_ = 0.5f * (hand_model.fingers[0].GetTipPosition() + hand_model.fingers[1].GetTipPosition());
	...
	protected void ContinueHardPinch() {
		...
		Quaternion delta_rotation = target_rotation * Quaternion.Inverse(active_object_.transform.rotation);
	...
	protected void ContinueSoftPinch() {
		...
		active_object_.GetComponent<Rigidbody>().AddTorque(strength * rotationFiltering * angle * axis, ForceMode.Acceleration);
	} // End of ContinueSoftPinch().
