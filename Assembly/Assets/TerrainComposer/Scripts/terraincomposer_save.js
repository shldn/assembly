...
function clear_terrain_list() 
{
	for (var count_terrain: int = 1;count_terrain < terrains.Count;++count_terrain) {
...
function select_automatic_step_resolution(preterrain1: terrain_class,prearea: area_class)
{
	...
	if (preterrain1.terrain.terrainData.alphamapResolution > resolution && color_output){resolution = preterrain1.prearea.colormap_resolution;prearea.resolution_mode = resolution_mode_enum.Colormap;}
				...
				switch(current_layer.output)
				{
					...
					case layer_output_enum.tree:
						...
						if (subfilter_value*current_layer.strength > 0)
						{
							...
							if (counter_y == 0 || x == 0) {
								UnityEngine.Random.seed = ((counter_y+1)*(x+1))+10000000+(count_layer);								
							}
							else {
								UnityEngine.Random.seed = (counter_y*x)+(count_layer);							
							}
	...
	{	
		switch (filter.type)
		{
			...
			case condition_type_enum.Image:
				...
				if (filter.preimage.splatmap) {splatmap = true;}
			...
			case condition_type_enum.Splatmap:
				if (filter.splatmap < preterrain.splat_alpha.Length-1) {
	...
	switch (filter.output)
	{
		...
		case condition_output_enum.add:
			if (current_layer.output == layer_output_enum.heightmap){filter_value += filter_input*filter_strength*subfilter_value;}
				else {filter_value += filter_input*filter_strength;}
		...
		if (filter[count_filter].type == condition_type_enum.Image) {
			if (filter[count_filter].preimage.image_list_mode == list_condition_enum.Terrain)
			{
				if (filter[count_filter].preimage.short_list) {
					...
					filter[count_filter].preimage.image[0] = AssetDatabase.LoadAssetAtPath(filter[count_filter].preimage.auto_search.get_file(terrains[terrain_index].tile_x,terrains[terrain_index].tiles.y-1-terrains[terrain_index].tile_z,terrains[terrain_index].index_old),Texture2D);
		...
		if (subfilter[count_subfilter].type == condition_type_enum.Image) {
			if (subfilter[count_subfilter].preimage.image_list_mode == list_condition_enum.Terrain)
			{
				if (subfilter[count_subfilter].preimage.short_list) {
					...
					subfilter[count_subfilter].preimage.image[0] = AssetDatabase.LoadAssetAtPath(subfilter[count_subfilter].preimage.auto_search.get_file(terrains[terrain_index].tile_x,terrains[terrain_index].tiles.y-1-terrains[terrain_index].tile_z,terrains[terrain_index].index_old),Texture2D);
					#endif
				}
				else {
...
function get_image_pixel(preimage: image_class,local_x: float,local_y: float): Color
{
	...
	// Debug.Log("pos: "+imagePosition+" color: "+color);
	return color1;
}
...
function set_area_resolution(preterrain1: terrain_class,prearea: area_class)
{
	...
	{
		prearea.step.x = preterrain1.terrain.terrainData.size.x/(preterrain1.prearea.colormap_resolution);
		prearea.step.y = preterrain1.terrain.terrainData.size.z/(preterrain1.prearea.colormap_resolution);
		preterrain1.splatmap_conversion.x = preterrain1.terrain.terrainData.size.x/(preterrain1.prearea.colormap_resolution-1);
		preterrain1.splatmap_conversion.y = preterrain1.terrain.terrainData.size.z/(preterrain1.prearea.colormap_resolution-1);
		preterrain1.splatmap_resolution = preterrain1.prearea.colormap_resolution;
		
		prearea.conversion_step = prearea.step;
		prearea.resolution = preterrain1.prearea.colormap_resolution;
	}
...
function load_grass(grassmap_index: int,terrain_index: int) 
{
	...
	for (var count_grass: int = 0;count_grass < length;++count_grass) {
		...
		for (var count_detail: int = 0;count_detail < length2;++count_detail) {
			...
			grass_detail[settings.grassmap[grassmap_index].grass_param[count_grass].prototype].detail [(count_detail-((count_detail/resolution2)*resolution2)),(count_detail/(resolution2+1))] += 
...
function stitch_splatmap()
{
	...
	for (var count_x: int = 0;count_x < terrains[0].tiles.x;++count_x)
	{
		for (var count_y: int = 0;count_y < terrains[0].tiles.y;++count_y)
		{
			terrain_index = count_y+(count_x*terrains[0].tiles.y);
			terrain_index2 = count_y+((count_x-1)*terrains[0].tiles.y);
			
			if (count_y != 0)
			{
				resolution = terrains[terrain_index-1].terrain.terrainData.alphamapResolution;
				if (terrains[terrain_index].terrain.terrainData.alphamapResolution != resolution){continue;}
				if (terrains[terrain_index].terrain.terrainData.splatPrototypes.Length != terrains[terrain_index-1].terrain.terrainData.splatPrototypes.Length) continue;
																	
				map = terrains[terrain_index-1].terrain.terrainData.GetAlphamaps(0,0,resolution,1);
 		...
 		for (prelayer3.x = prelayer3.prearea.area.x+prelayer3.break_x_value;prelayer3.x <= prelayer3.prearea.area.xMax;prelayer3.x += prelayer3.prearea.step.x)
 		{
			...
			for (var count_layer: int = 0;count_layer < prelayer3.layer.Count;++count_layer)
			{
				...
				if (subfilter_value*current_layer.strength*filter_strength > 0)
				{
					...
					if (counter_y == 0 || x == 0) {
						UnityEngine.Random.seed = ((counter_y+1)*(x+1))+10000000*(count_layer+1);								
					}
					else {
						UnityEngine.Random.seed = (counter_y*x)*(count_layer+1);							
					}
...
function convert_terrains_to_mesh(): String
{
	...
	for (count_terrain = 0;count_terrain < terrains.Count;++count_terrain) {
		...
		vertex[count_terrain].vertices = mesh[count_terrain].vertices;
	}
	...
	for (count_terrain = 0;count_terrain < terrains.Count;++count_terrain) {
		...
		for (y = 1;y < resolution-1;++y) {
			for (x = 1;x < resolution-1;++x) {
				pos = (y*resolution)+x;
				normal = calc_normal(conversion.x,vertex[count_terrain],pos,resolution);
	...
	return "Converting "+terrains.Count.ToString()+" terrains to meshes done";
}
