...
class TerrainComposer extends EditorWindow 
{
    ...
    */
	...
	function OnFocus()
	{
		if (!TerrainComposer_Scene){TerrainComposer_Scene = GameObject.Find("TerrainComposer_Save");}
	...
	function OnGUI() 
    {
        ...
        EditorGUILayout.LabelField("Object",EditorStyles.miniBoldLabel,GUILayout.Width(52));
	...
	{       
		...
		*/
		...
		if (script.settings.update_display)
	    {
			...
			EditorGUILayout.LabelField("Final "+read_version().ToString("F3"));
			GUI.color = Color.white;
			...
			else {EditorGUILayout.LabelField("Final "+script.settings.new_version.ToString("F3")+"");}
	    ...
	    if (script.settings.database_display)
        {
	        ...
	        if (GUILayout.Button("<Fix Database>",GUILayout.Width(120)) && key.shift)
	        {
	        		...
	        		Undo.RecordObject(script,"Fix Database");
	        	#else
	    ...
	    if (script.settings.project_prefab) {
	    	...
	    	if (parent_prefab) {
	        	...
	        	if (GUILayout.Button("<Revert>",GUILayout.Width(70)) && key.shift) {
	        		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        			Undo.RecordObject(TerrainComposer_Scene,"Revert Project Prefab");
	        		#else
	        			Undo.RegisterUndo(TerrainComposer_Scene,"Revert Project Prefab");
	        		#endif
...
//		        EditorGUILayout.EndHorizontal();
		...
		if (global_script.settings.example_display) {
		    ...
		    gui_changed_old = GUI.changed;
		   	...
		   	if (GUI.changed) {
		   		if (global_script.settings.example_tiles*Mathf.Pow(2,7+global_script.settings.example_resolution) > 8192) {
		   			do {
		   				global_script.settings.example_tiles -= 1;
		   			}
		   			while (global_script.settings.example_tiles*Mathf.Pow(2,7+global_script.settings.example_resolution) > 8192);
		   		}
		   	}
		   	GUI.changed = gui_changed_old;
		   	...
		   	GUI.changed = false;
		   	GUI.color = Color.green;
		   	EditorGUILayout.LabelField("Terrain Tiles",GUILayout.Width(145));
		   	GUI.color = Color.white;
		   	global_script.settings.example_tiles = EditorGUILayout.Slider(global_script.settings.example_tiles,1,10);	
		   	if (GUI.changed) {
		   		if (global_script.settings.example_tiles*Mathf.Pow(2,7+global_script.settings.example_resolution) > 8192) {
		   			do {
		   				global_script.settings.example_resolution -= 1;
		   			}
		   			while (global_script.settings.example_tiles*Mathf.Pow(2,7+global_script.settings.example_resolution) > 8192);
		   		}
		   	}
		   	GUI.changed = gui_changed_old;
		   	EditorGUILayout.LabelField(global_script.settings.example_tiles.ToString()+"X"+global_script.settings.example_tiles.ToString(),GUILayout.Width(50));
		   	EditorGUILayout.EndHorizontal();
			...
			if (GUILayout.Button("Create Lighting",GUILayout.Width(145))) {
				...
				RenderSettings.fogDensity = 0.0012;
 		...
 		if (script.terrains_foldout)
	    {
        	...
        	if (GUILayout.Button(GUIContent("-",tooltip_text),GUILayout.Width(25)) && script.terrains.Count > 1)
        	{
        		if (key.control) {
	        		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        			Undo.RecordObject(script,"Terrain Erase");
		        	#else
	        	...
	        	if (script.quick_tools_foldout)
	        	{
				    ...
				    if (script.settings.mesh_button)
				    {
				    	...
				    	EditorGUILayout.BeginHorizontal();
			       		...
			       		if (script.terrains.Count < 2){button_mesh_text = "Convert Terrain to Mesh";} else {button_mesh_text = "Convert Terrains to Meshes";}
			       		if (GUILayout.Button(button_mesh_text,GUILayout.Width(170))) {
			       			this.ShowNotification(GUIContent(script.convert_terrains_to_mesh()));
			       		}
			       		EditorGUILayout.EndHorizontal();
			       	}
			        		...
			        		if (current_terrain.terrain)
			        		{
				        		...
				        		if (GUILayout.Button(GUIContent("<Set>",tooltip_text),GUILayout.Width(75)))
				        		{
				        			if (key.shift) {
				        				...
				        				script.check_synchronous_terrain_size(current_terrain);
				        				if (script.terrains.Count == 1){fit_all_terrains();}
				        			}
				        			else {
		        		...
		        		if (current_terrain.splat_foldout)
		        		{
		        			...
		        			if (GUILayout.Button(GUIContent("-",tooltip_text),GUILayout.Width(25)))
		        			{
		        				if (key.control) {
			        				...
			        				{
			        					#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Splat Erase");
	        							#else
		    						...
		    						{
		    							#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Splats Erase");
							        	#else
			        		...
			        		EditorGUILayout.EndHorizontal();
		        				...
		        				for (count_splat = 0;count_splat < current_terrain.splatPrototypes.Count;++count_splat)
			        			{
			        				...
			        				if (count_splat > -1)
			        				{
								    	...
								    	if (GUILayout.Button(GUIContent("<",tooltip_text),GUILayout.Width(25)) && count_splat > 0)
				        				{
				        				...
				        				if (key.type == EventType.Repaint) {current_terrain.splat_rect = GUILayoutUtility.GetLastRect();}
							           	 
				        				if (count_splat == 0 && script.settings.colormap)
								    	{
								    		if (select_window) {GUI.backgroundColor = Color.green;}
								    		if (GUI.Button(Rect(current_terrain.splat_rect.x,current_terrain.splat_rect.y+20,70,17),"Select"))
								    		{
				        				...
				        				if (GUILayout.Button(GUIContent(">",tooltip_text),GUILayout.Width(25)) && count_splat < current_terrain.splatPrototypes.Count-1){script.swap_terrain_splat(current_terrain,count_splat,count_splat+1);script.check_synchronous_terrain_textures(current_terrain);}		 
							            if (global_script.settings.tooltip_mode != 0)
								        {
		        		...
		        		if (current_terrain.tree_foldout)
		        		{
		        			...
		        			if (GUILayout.Button(GUIContent("-",tooltip_text),GUILayout.Width(25)))
		        			{
		        				if (key.control) {
			        				...
			        				{
			        					#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Tree Erase");
							        	#else
		        			...
		        			for (count_tree = 0;count_tree < current_terrain.treePrototypes.Count;++count_tree)
		        			{
		        				...
		        				if (current_terrain.treePrototypes[count_tree].foldout)
		        				{
					        		...
					        		if (GUILayout.Button(GUIContent(">Set All",tooltip_text),EditorStyles.miniButtonMid,GUILayout.Width(65)))
					        		{
					        			...
					        			{
					        				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        									Undo.RecordObject(script,"Set all Trees Settings");
								        	#else
	    								...
	    								{
	    									#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        									Undo.RecordObject(script,"Set Trees all Terrains Settings");
								        	#else
		        		...
		        		if (current_terrain.detail_foldout)
		        		{
		        			...
		        			if (GUILayout.Button(GUIContent("-",tooltip_text),GUILayout.Width(25)))
		        			{
		        				if (key.control) {
			        				...
			        				{
			        					#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Grass Erase");
							        	#else
							        		Undo.RegisterUndo(script,"Grass Erase");
							        	#endif
	...
	function draw_prelayer(prelayer: prelayer_class,space: int,text: String,layer_minimum: int)
	{
	    ...
	    if (prelayer.foldout)
	    {
			...
			if (GUILayout.Button(GUIContent("<Sort>",tooltip_text),GUILayout.Width(55)))
        	{
        		if (key.shift) {
        			#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        			Undo.RecordObject(script,"Sort Layers");
		        	#else
        	...
        	if (global_script.settings.tooltip_mode != 0)
			{
				tooltip_text = "Activate/Deactivate help mode";
			}
			if (global_script.settings.tooltip_mode != 0)
			{
	        	...
	        	if (script.description_display)
        		{
	        		...
	        		if (new_description)
				    {
				       	if (count_description == new_description_number)
				       	{
					       	...
					       	{
					       		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        						Undo.RecordObject(script,"New LayerGroup");
				        		#else
	           	...
	           	if (current_layer.output == layer_output_enum.heightmap || current_layer.output == layer_output_enum.color || current_layer.output == layer_output_enum.splat)
	           	{
					if (script.terrains.Count > 1)
					{
	           		...
	           		if (current_layer.output == layer_output_enum.heightmap)
	           		{
						...
						current_layer.strength = EditorGUILayout.Slider(current_layer.strength,0,4);
						EditorGUILayout.EndHorizontal();
					}
				}
					...
					if (current_layer.color_output.foldout)
	           		{
        				...
        				if (GUILayout.Button("-",GUILayout.Width(25)) && current_layer.color_output.precolor_range.Count > 1)
        				{
        					if (key.control) {
		    					#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        						Undo.RecordObject(script,"ColorGroup Erase");
					        	#else
	        		...
	        		if (current_layer.splat_output.foldout)
	        		{
		        		...
		        		if (GUILayout.Button("-",GUILayout.Width(25)) && current_layer.splat_output.splat.Count > 0)
		        		{
		        			if (key.control) {
			        			...
			        			{
			        				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
						        		Undo.RecordObject(script,"Erase Splat");
						        	#else
			        			...
			        			{
			        				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        							Undo.RecordObject(script,"Erase Splats");
						        	#else
				        ...
				        EditorGUILayout.BeginHorizontal();
			           	GUILayout.Space(space+45);
				        EditorGUILayout.LabelField("Strength",GUILayout.Width(65));
						current_layer.strength = EditorGUILayout.Slider(current_layer.strength,0,1);
						EditorGUILayout.EndHorizontal();
	           				...
	           				if (current_layer.splat_output.mix_mode == mix_mode_enum.Single && count_splat < current_layer.splat_output.splat.Count-1)
				            {
					        	if (global_script.settings.color_scheme)
					         	...
					         	EditorGUILayout.BeginHorizontal();
					           	...
					           	EditorGUILayout.EndHorizontal();
					           	if (global_script.settings.color_scheme){GUI.color = color_splat;}
					           	if (current_layer.splat_output.splat[count_splat] < script.terrains[current_layer.splat_output.splat_terrain].splatPrototypes.Count) {GUILayout.Space(16);}
					         }
	        		...
	        		if (current_layer.tree_output.foldout)
	        		{
		        		...
		        		if (GUILayout.Button("-",GUILayout.Width(25)))
		        		{
		        			if (key.control) {
			        			...
			        			{
			        				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        							Undo.RecordObject(script,"Erase Tree");
						        	#else
			        			...
			        			{
			        				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Erase Trees");
							        	#else
			            ...
			            for (count_tree = 0;count_tree < current_layer.tree_output.tree.Count;++count_tree)
		           		{
							...
							{
		           				if (GUILayout.Button("<",GUILayout.Width(25)) && count_tree > 0) {
			           			...
			           			if (GUILayout.Button(">",GUILayout.Width(25)) && count_tree < current_layer.tree_output.tree.Count-1) {
			           				current_layer.tree_output.swap_tree(count_tree,count_tree+1);
			           				if (script.generate_auto){generate_auto();}
			           			}
			           			if (GUILayout.Button("+",GUILayout.Width(25)))
		           				...
		           				if (current_tree.scale_foldout)
		           				{
									...
									EditorGUILayout.EndHorizontal();
				           			...
				           			if (current_layer.tree_output.tree.Count > 1)
				           			{
						           		...
						           		{
						           			#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
								        		Undo.RecordObject(script,"Set All Scale Trees");
								        	#else
				           		...
				           		{ 
					        		...
					        		if (current_tree.distance_mode == distance_mode_enum.Square)
									{
						           	...
						           	if (current_layer.tree_output.tree.Count > 1)
						           	{
					           			...
					           			{
					           				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        									Undo.RecordObject(script,"Set All Distance Trees");
								        	#else
	        	...
	        	if (current_layer.grass_output.foldout && script.terrains[current_layer.grass_output.grass_terrain])
	        	{
		        	...
		        	if (GUILayout.Button("-",GUILayout.Width(25)))
		        	{
		        		if (key.control) {
			        		...
			        		{
			        			#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        						Undo.RecordObject(script,"Erase Grass");
					        	#else
							...
							{
								#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        						Undo.RecordObject(script,"Erase Grasses");
						        #else
			        ...
			        for (count_grass = 0;count_grass < current_layer.grass_output.grass.Count;++count_grass)
		           	{
		           		...
		           		if (current_layer.grass_output.grass[count_grass].prototypeindex < script.terrains[current_layer.grass_output.grass_terrain].detailPrototypes.Count)
						{
							...
							if (script.terrains[current_layer.grass_output.grass_terrain].detailPrototypes[current_grass.prototypeindex].prototype && script.terrains[current_layer.grass_output.grass_terrain].detailPrototypes[current_grass.prototypeindex].usePrototypeMesh) {
								...
								if (script.terrains[current_layer.grass_output.grass_terrain].detailPrototypes[current_grass.prototypeindex].previewTexture) {
									...
									EditorGUILayout.LabelField(script.terrains[current_layer.grass_output.grass_terrain].detailPrototypes[current_grass.prototypeindex].prototype.name);
								}
							}
						...
						GUI.changed = gui_changed_old;
		           		...
		           		EditorGUILayout.EndHorizontal();
		           		if (draw_grass_texture){GUILayout.Space(35);}
	       		...
	       		if (current_layer.object_output.foldout)
	       		{
		        	...
		        	if (GUILayout.Button("-",GUILayout.Width(25)) && current_layer.object_output.object.Count > 1)
		        	{
		        		if (key.control) {
			        		...
			        		{
			        			#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        						Undo.RecordObject(script,"Ease Object");
					        	#else
					        ...
					        {
					           	#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        						Undo.RecordObject(script,"Ease Objects");
					        	#else
			        ...
			        if (current_layer.object_output.search_active)
			        {
						...
						if (GUILayout.Button("<Clear>",GUILayout.Width(70)))
			        	{
			        		if (key.shift) {
			        			#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        						Undo.RecordObject(script,"Clear Objects");
					        	#else
					        ...
					        if (current_object.settings_foldout)
					        {
						        ...
						        */
						        ...
						        if (global_script.settings.tooltip_mode != 0)
				           			...
				           			if (current_layer.object_output.object.Count > 1)
				           			{
					           			...
					           			{
					           				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
								        		Undo.RecordObject(script,"Set All Settings Objects");
								        	#else
							    ...
							    if (current_object.prelayer_created)
						       	{
									...
									{
										#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Erase Layer Level");
							        	#else
		    	     		...
		    	     		if (current_object.object_material.foldout)
		    	     		{
			    	     		...
			    	     		if (GUILayout.Button("+",GUILayout.Width(25))){current_object.object_material.add_material(current_object.object_material.material.Count);}	 
			           			if (GUILayout.Button("-",GUILayout.Width(25)) && current_object.object_material.material.Count > 1)
			           			{
			           				if (key.control) {
				           				#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Erase Material");
							        	#else
		    	     		...
		    	     		if (current_object.transform_foldout)
		    	     		{
						        ...
						        if (current_layer.object_output.object.Count > 1)
						        {
							        ...
							        {
							        	#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Set All Transforms Objects");
							        	#else
					        ...
					        EditorGUILayout.EndHorizontal();	
				           	...
				           	if (current_object.rotation_foldout)
				           	{
						  		...
						  		if (current_layer.object_output.object.Count > 1)
						    	{
							        ...
							        {
							        	#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Set All Rotation Objects");
							        	#else
				           	...
				           	{ 
						        ...
						        if (current_layer.object_output.object.Count > 1)
						        {
							        ...
							        {
							        	#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        								Undo.RecordObject(script,"Set All Distance Objects");
							        	#else
		...
		if (prefilter.foldout)
		{
			...
			if (GUILayout.Button("-",GUILayout.Width(25)) && prefilter.filter_index.Count > 0)
			{
				if (key.control) {
					...
					{
						#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Erase Filter");
			        	#else
					...
					{
						#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Erase FilterS");
			        	#else
			...
			if (position_prefilter == 1)
			{
				if (current_layer.tree_output.tree.Count > 1)
				{
					...
					{
						#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Set All Tree Filters");
			        	#else
		...
		if (current_filter.curve_foldout)
		{
			for (var count_curve: int = 0;count_curve < precurve_list.Count;++count_curve)
			{
				...
				if (GUILayout.Button(precurve_list[count_curve].curve_text,EditorStyles.miniButtonMid,GUILayout.Width(63)))
				{
					...
					{
						#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Default Curve");
			        	#else
				...
				if (GUILayout.Button("-",EditorStyles.miniButtonMid,GUILayout.Width(25)))
				{
					if (key.control) {
						...
						{
							#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        					Undo.RecordObject(script,"Erase Curve");
				        	#else
		...
		if (current_filter.presubfilter.foldout)
		{
		    ...
		    if (GUILayout.Button("-",GUILayout.Width(25)) && current_filter.presubfilter.subfilter_index.Count > 0)
		    {
		    	if (key.control) {
			    	...
			    	{
			    		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Erase subfilter");
			        	#else
					...
					{
						#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Erase subilters");
			        	#else
	...
	function draw_area(prelayer_number: int,prearea: area_class,current_terrain: terrain_class,space: int,world_area: boolean)
	{
	    ...
	    if (prearea.foldout)
	    {
			...
			*/
			...
			EditorGUILayout.BeginHorizontal();
			GUILayout.Space(45+space);
			EditorGUILayout.LabelField("Colormap Resolution",GUILayout.Width(135));
			gui_changed_old = GUI.changed;
			gui_changed_window = GUI.changed; GUI.changed = false;
			prearea.colormap_resolution = EditorGUILayout.IntField(prearea.colormap_resolution,GUILayout.Width(55));	
			if (GUI.changed)
			{
				if (prearea.colormap_resolution < 8){prearea.colormap_resolution = 8;}	
			}
			GUI.changed = gui_changed_old;
			// prearea.object_resolution_active = EditorGUILayout.Toggle(prearea.object_resolution_active,GUILayout.Width(25));
			EditorGUILayout.EndHorizontal();
		...
		if (precolor_range.foldout)
		{
			...
			if (GUILayout.Button("-",GUILayout.Width(25)) && precolor_range.color_range.Count > minimum)
			{
				if (key.control) {
		        	...
		        	{
		        		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Erase Color");
			        	#else
					...
					{
						#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        				Undo.RecordObject(script,"Erase Colors");
			        	#else
        	...
        	if (call_from == 1 || call_from == 2)
        	{
				if (GUILayout.Button(GUIContent("<Detect>",tooltip_text),GUILayout.Width(75)))
			    {
			    	...
			    	#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        			Undo.RecordObject(script,"Detect Colors");
		        	#else
	...
	{
		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
			Undo.RecordObject(script,"LayerGroup Erase");	        	
    	#else
	...
	{
	    #if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        Undo.RecordObject(script,"Layer Erase");
		#else
	...
	{
		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        Undo.RecordObject(script,"Filter Erase");
		#else
	...
	{
		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
    		Undo.RecordObject(script,"Subfilter Erase");
    	#else
	...
	{
		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        Undo.RecordObject(script,"Color Range Erase");
    	#else
	...
	{
		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        Undo.RecordObject(script,"Object Erase");
    	#else
	...
	*/
	...
	{
		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        Undo.RecordObject(script,"Load Layer");
    	#else
	...
	{
		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        Undo.RecordObject(script,"Load LayerGroup");
		#else
    ...
    {        
    	...
    	{
    		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        	Undo.RecordObject(script,"New Layer");
        	#else
    ...
    {        
    	...
    	{
	    	#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        	Undo.RecordObject(script,"Sort LayerGroup");
        	#else
    ...
    function curve_menu(obj: curve_menu_arguments_class)
    {
    	...
    	{
    		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        	Undo.RecordObject(script,"Set Zero Curve");
        	#else
    	...
    	{
    		#if !UNITY_3_4 && !UNITY_3_5 && !UNITY_4_0 && !UNITY_4_1 && !UNITY_4_2
	        	Undo.RecordObject(script,"Set Default Curve");
        	#else
	...
	function auto_search_list(preimage: image_class)
	{
		...
		// {
			...
			if (!preimage.short_list) {
		    	...
		    	tiles = Mathf.Sqrt(script.terrains.Count);
		    }
			...
			for (var count_terrain: int = 0;count_terrain < script.terrains.Count;++count_terrain)
			{
				x = script.terrains[count_terrain].tile_x;
				y = script.terrains[0].tiles.y-script.terrains[count_terrain].tile_z-1;
	...
	function create_terrain(preterrain: terrain_class,length: int,start_number: int,name_number: int,terrain_path: String,terrain_parent: Transform)
	{
		...
		*/
		...
		for (var count_terrain1: int = 0;count_terrain1 < length;++count_terrain1)
		{
		    ...
		    terrain.name = script.terrain_scene_name+(count_terrain1+name_number);
		    terrain.terrainData = terrainData;
		    var path: String = terrain_path;
		    path = "Assets"+path.Replace(Application.dataPath,String.Empty);
		    path += "/"+script.terrain_asset_name+(count_terrain1+name_number)+".asset";
	...
	function slice_terrains()
	{
		...
		for (count_terrain1 = 0;count_terrain1 < script.terrains.Count;++count_terrain1)
		{
			...
			{
				this.ShowNotification(GUIContent("Grass resolution of "+script.terrains[count_terrain1].name+" is 32, this cannot be made smaller, read console suggestion"));
		...
		script.fit_terrain_tiles(script.terrains[0],true);
	...
	function import_terrain_wc(name: String) 
	{
		...
		// add_layer(prelayer: prelayer_class,layer_position,description_position: int,layer_index: int);
	 	for (var count_terrain: int = 0;count_terrain < map_window.terrain_region.area[0].terrains.Count;++count_terrain) {
	 		terrain_index = script.calc_terrain_index2(tiles,map_window.create_area.tiles);
	 		
	 		script.add_terrain(script.terrains.Count);
	 		copy_terrain2(script.terrains[count_terrain],map_window.terrain_region.area[0].terrains[terrain_index],map_window.create_area.do_image);
	 		...
	 		++tiles.y;
	 		if (tiles.y > map_window.create_area.tiles.y-1) {++tiles.x;tiles.y = 0;}
	 	}
	...
	function copy_terrain2(preterrain1: terrain_class,preterrain2: terrain_class2,splat: boolean)
	{
		...
		preterrain1.tile_z = map_window.create_area.tiles.y-1-preterrain2.tile_z;
	...
	function draw_measure_tool()
	{
       	...
       	if (script.measure_tool_foldout || script.measure_tool_undock)
       	{
			...
			{	        		
	       	...
	       	EditorGUILayout.LabelField("("+(script.settings.terrainMinDegree/heightmap_scale.y).ToString("F3")+")",GUILayout.Width(70));
	       	EditorGUILayout.EndHorizontal(); 
	       	...
	       	EditorGUILayout.LabelField("("+(script.settings.terrainMaxDegree/heightmap_scale.y).ToString("F3")+")",GUILayout.Width(70));
	       	EditorGUILayout.EndHorizontal(); 
	...
	function create_example(path: String,load_only: boolean,projectName: String)
	{
		...
		if (script.create_pass == 4) {
			...
			script.create_pass = -1;
			if (script.tree_output || script.grass_output) {
				generate_startup();
			}
		}
	}
	...
	function load_example_presets()
	{
		...
		script.calc_terrain_needed_tiles();
		if (script.terrain_instances > 0) {
			create_terrain(current_terrain,script.terrain_instances,script.terrains.Count,script.terrains.Count,"/TerrainComposer Examples/Terrains",script.terrain_parent);
		}
		...
		script.terrains[0].prearea.tree_resolution = 512;
		script.terrains[0].prearea.object_resolution_active = true;
		script.terrains[0].prearea.object_resolution = 512;
