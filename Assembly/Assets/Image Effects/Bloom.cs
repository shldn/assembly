...
public class Bloom : PostEffectsBase
{
	...
	private void  AddTo ( float intensity_ ,   RenderTexture from1 ,   RenderTexture to1 )
	{
		screenBlend.SetFloat ("_Intensity", intensity_);
		to1.MarkRestoreExpected(); // additive blending, RT restore expected
		Graphics.Blit (from1, to1, screenBlend, 9); 		
	}
	
	private void  BlendFlares ( RenderTexture from1 ,   RenderTexture to1 )
	{
		...
		to1.MarkRestoreExpected(); // additive blending, RT restore expected
		Graphics.Blit (from1, to1, lensFlareMaterial);			
	}

	private void  BrightFilter ( float thresh ,   RenderTexture from1 ,   RenderTexture to1 )
	{
		brightPassFilterMaterial.SetVector ("_Threshhold", new Vector4 (thresh, thresh, thresh, thresh));
		Graphics.Blit (from1, to1, brightPassFilterMaterial, 0);			
	}

	private void  BrightFilter ( Color threshColor ,   RenderTexture from1 ,   RenderTexture to1 )
	{
		brightPassFilterMaterial.SetVector ("_Threshhold", threshColor);
		Graphics.Blit (from1, to1, brightPassFilterMaterial, /*1*/0);
	}	
	
	private void  Vignette ( float amount ,   RenderTexture from1 ,   RenderTexture to1 )
	{
		if(lensFlareVignetteMask) {
			screenBlend.SetTexture ("_ColorBuffer", lensFlareVignetteMask);
			to1.MarkRestoreExpected(); // using blending, RT restore expected
			Graphics.Blit (from1 == to1? null : from1, to1, screenBlend, from1 == to1? 7 : 3); 				
		} 
		else if (from1 != to1)
		{
			Graphics.SetRenderTarget (to1);
			GL.Clear(false, true, Color.black); // clear destination to avoid RT restore
			Graphics.Blit (from1, to1);
		}
	}
